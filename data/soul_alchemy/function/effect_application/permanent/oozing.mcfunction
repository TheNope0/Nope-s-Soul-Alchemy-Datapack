execute if entity @s[tag=permanent_oozing] run return run tellraw @s {"translate": "info.n_soul_alchemy.effect_already_permanent","color": "gray"}


execute if entity @s[tag=deactivate_oozing] run scoreboard players add @s npe_soul_points 2
execute if entity @s[tag=deactivate_oozing] run tellraw @s {"translate": "info.n_soul_alchemy.effect_permanent","color": "gray"}
