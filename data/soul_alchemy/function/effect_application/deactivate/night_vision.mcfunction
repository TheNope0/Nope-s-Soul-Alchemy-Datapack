execute if entity @s[tag=deactivate_night_vision] run return run tellraw @s {"translate": "info.soul_alchemy.effect_already_disabled","color": "gray"}


execute if entity @s[tag=permanent_night_vision] run scoreboard players add @s npe_soul_points 8
execute if entity @s[tag=permanent_night_vision] run tellraw @s {"translate": "info.soul_alchemy.effect_disabled","color": "gray"}
